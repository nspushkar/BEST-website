const codeSnippets = {
  ...
  cv2.destroyAllWindows()
    ...
    step1: {
      code: `def classify_color(hsv_value):
      h, s, v = hsv_value
      if (0 <= h <= 10) or (160 <= h <= 180):
          return "Hot"  # Red
      elif 100 <= h <= 140:
    ...
    color: {
      code: `# Capture image from webcam
  cap = cv2.VideoCapture(0)
  ...
  while True:
    ...
    hsv: {
      code: `# Convert the image from BGR to HSV color space
      hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
      `,
      language: 'python'
    },
    step3: {
      code: `  cv2.putText(frame, f"Classification: {classification}", (10, 30),
                  cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
